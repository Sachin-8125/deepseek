...
const OPENAI_BASE_URL =
    process.env.NVIDIA_BASE_URL || process.env.OPENAI_BASE_URL || "https://integrate.api.nvidia.com/v1";
const OPENAI_MODEL =
    process.env.NVIDIA_MODEL || process.env.OPENAI_MODEL || "deepseek-ai/deepseek-v4-flash";

if (!OPENAI_API_KEY) {
    // Fail fast with a clear error. This avoids confusing 404s downstream.
    console.error("[AI Router] Missing API key. Set DEEPSEEK_API_KEY (or provider-specific key).");
}
...
const OPENAI_BASE_URL_FALLBACK = (() => {
    const trimmed = (OPENAI_BASE_URL || "").trim().replace(/\/+$/, "");
    if (!trimmed) return "";
    return `${trimmed}/openai`;
})();

const openaiFallback = OPENAI_BASE_URL_FALLBACK
    ? new OpenAI({
          apiKey: OPENAI_API_KEY,
          baseURL: OPENAI_BASE_URL_FALLBACK,
      })
    : null;

async function createChatCompletionWithFallback({ messages, model, temperature, max_tokens }) {
    try {
        ...
        if (status === 404 && openaiFallback) {
            console.warn("[AI Router] 404 from primary baseURL; retrying with fallback baseURL:", {
                primaryBaseURL: OPENAI_BASE_URL,
                fallbackBaseURL: OPENAI_BASE_URL_FALLBACK,
                model,
            });
            ...
            try {
                ...
                // Enrich error so outer handler can report which baseURL failed.
                fallbackError.baseURLUsed = OPENAI_BASE_URL_FALLBACK;
                throw fallbackError;
            }
        }

        error.baseURLUsed = OPENAI_BASE_URL;
        throw error;
    }
}
...
Answer ONLY: YES or NO`;
        console.log("[AI Router] Reference classifier request:", {
            baseURL: OPENAI_BASE_URL,
            model: OPENAI_MODEL,
        });

        const referenceClassifierResponse = await createChatCompletionWithFallback({
        ...
        if (searchSelected) {
            ...
            console.log("[AI Router] Web search classifier request:", {
                baseURL: OPENAI_BASE_URL,
                model: OPENAI_MODEL,
            });
...
Respond now:`;
        ...
        // Use configured model for both classifier + main completion.
        // (Kept as `modelToUse` for minimal code changes.)
        ...
        console.log("[AI Router] Main completion request:", {
            baseURL: OPENAI_BASE_URL,
            model: modelToUse,
        });
        ...
        console.error("[AI Router] LLM request failed:", {
            message: error?.message,
            status: error?.status,
            baseURL: error?.baseURLUsed || OPENAI_BASE_URL,
            model: OPENAI_MODEL,
        });

        const status = error?.status;

        // If the provider returns a 404, it's typically "model not found" or "wrong baseURL/path".
        if (status === 404) {
            return NextResponse.json({
                success: false,
                error: "LLM provider returned 404. Check base URL and model name for this provider.",
                provider: "openai-compatible",
                baseURL: error?.baseURLUsed || OPENAI_BASE_URL,
                model: OPENAI_MODEL,
                hint:
                    "If you’re using NVIDIA Integrate, NVIDIA may require a different path than /v1 or /v1/openai. Update NVIDIA_BASE_URL accordingly.",
            });
        }
